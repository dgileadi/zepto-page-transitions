...
f||(f=document.title);c.not("[data-title]").data("title",f);if(f=a.data("title"))document.title=f;c.hide();d=(d=d?b(d):null)||a;d.addClass("ui-page-active");c.trigger("pageinit");return d},to:function(a,b,f){b=b||l.defaultPageTransition;f||(f=!1);e={transition:b,reverse:f};window.location.hash="#"+a},hijackLinks:function(){return this.each(function(){var a=b(this);if("back"==a.data("rel")){var d=function(a){window.history.back();a.preventDefault()};a.on("click",d);a.on("tap",d)}else{var f=a.attr("data-href")||
a.attr("href")||a.attr("action")||"#";if("#"===f.charAt(0)){if(b('a[name="'+f.slice(1)+'"]').length){v[f]=!0;return}}else f="#"+f,a.is("a")?a.attr("href",f):a.attr("action")&&a.attr("action",f),a.attr("data-href")&&a.attr("data-href",f);a.is("a")?d=function(){e={element:a}}:a.is("form")?(d=function(b){e={element:a};window.location.href=f;b.preventDefault()},a.on("submit",d)):d=function(){e={element:a};window.location.href=f};a.is("form")||(a.on("click",d),a.on("tap",d))}})},changePage:function(a,
d,f){var c={toPage:a,back:f},h=b.Event("pagebeforechange");b(this).trigger(h,c);if(!h.defaultPrevented){a=c.toPage;f=c.back;var h="string"===typeof a?a:a.url,g=null,e=b("div.ui-page-active"),m=!1;if("string"===typeof a&&"#"===a.charAt(0)){var k=b(a.replace(/[:\.\+\/]/g,"_"));k.length?(b(this).transition("perform",e,k,d,f,c),m=!0):!l.domCache&&q[a]?(g=a,a=q[a]):a=a.slice(1)}if(!m){var j={href:h,element:b(this),back:f},h=b.Event("pagebeforeload");b(this).trigger(h,j);var n=b(this);h.defaultPrevented||
(b(this).transition("load",a,j,function(a,h,k){h=b("<div />");h.html(a);b(document.body).append(h);a=b(h).transition("init",j,g,k);b(n).transition("perform",e,a,d,f,c)}),m=!0)}m||b(this).trigger("pagechangefailed",c)}},load:function(a,d,f){a="string"===typeof a?{url:a,dataType:"html",global:!1}:a;a.url||(a.url=window.location.href);a.success=function(c,h,g){d.xhr=g;d.textStatus=h;u=a.url;l.domCache||b('div[data-role="page"]').not('[data-dom-cache="true"]').addClass("transition-recyclable");g=c.search(/<body/i);
var e=c;h=c;var m;-1!=g&&(e=c.slice(0,g),g=c.indexOf(">",g),bodyEnd=c.search(/<\/body>/i),h=c.slice(g+1,bodyEnd));(g=e.match(/<title>(.+)<\/title>/im))&&(m=g[1]);if(window.location.hash){g=window.location.pathname;var k=window.location.hash.slice(1),e="",j=k.lastIndexOf("/");if(-1!=j&&(e=k.slice(0,j+1),"/"==e.charAt(0))){"/"==!g.charAt(0)&&(g="/"+g);j=k=1;do j=e.indexOf("/",j)+1,j>k&&e.slice(0,j)==g.slice(0,j)&&(k=j);while(0<j&&j==k);var n="",j=k;do j=g.indexOf("/",j+1),-1!=j&&(n+="../");while(-1!=
